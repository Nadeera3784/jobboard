...
import { ElasticSearchQueryBuilder, ExpressionsFactory } from '../query-builder';
...
export class ElasticProductsQueryBuilderHelper {
    static createBuilder(filters: CommonFilterInterface[], strict = true): ElasticSearchQueryBuilder {
    ...
    private static buildFieldQuery(filter: CommonFilterInterface, path: string | null = null) {
        if (!path) {
    ...
    private static buildScalarExpression(filter: CommonFilterInterface, fieldName: string) {
    ...
    private static buildExpressionsForMultipleValues(field: string, value: any[], path: string | null = null) {
        const conditions = [];
    ...
    private static getExpressionType(condition: FilterFieldCondition, isArray = false): ElasticSearchExpressionsEnum | null {
        switch (condition) {
